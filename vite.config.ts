...
import { notFoundHtml, resolveSiteUrl, robotsTxt, seoHeadTags, sitemapXml } from './scripts/seo.ts';

/** Files generated by the SEO plugin; they are crawler-facing and never precached. */
const SEO_FILES = new Set(['robots.txt', 'sitemap.xml', '404.html']);
...
/**
 * Search-engine metadata derived from src/config/branding.ts: title, description,
 * Open Graph / Twitter cards and JSON-LD in index.html, plus robots.txt, the
 * static 404 page and – when the optional SITE_URL build variable is set – the
 * canonical URL and sitemap.xml. In dev the generated files are served from memory.
 */
function seoPlugin(siteUrl: string | null): Plugin {
  const files = (): Array<[name: string, type: string, source: string]> => {
    const list: Array<[string, string, string]> = [
      ['robots.txt', 'text/plain; charset=utf-8', robotsTxt(siteUrl)],
      ['404.html', 'text/html; charset=utf-8', notFoundHtml()],
    ];
    if (siteUrl) list.push(['sitemap.xml', 'application/xml; charset=utf-8', sitemapXml(siteUrl)]);
    return list;
  };
  return {
    name: 'flareqr-seo',
    transformIndexHtml: () => seoHeadTags(siteUrl),
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        const pathname = (req.url ?? '').split('?')[0] ?? '';
        const match = files().find(([name]) => pathname === `/${name}`);
        if (!match) return next();
        res.statusCode = match[0] === '404.html' ? 404 : 200;
...
export default defineConfig(({ mode }) => {
  const siteUrl = resolveSiteUrl(loadEnv(mode, process.cwd(), '').SITE_URL);
  return {
    plugins: [
      react(),
      tailwindcss(),
